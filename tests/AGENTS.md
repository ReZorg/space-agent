...
- `test-lane-catalog.json`: canonical lane map that classifies each `tests/*_test.mjs` entry as `unit`, `integration`, `desktop_e2e`, or `stress_perf`, marks heavy and flaky-prone coverage, defines lane timeout plus retry policy, and keeps explicit per-lane `tests` selections used by local scripts and CI workflows
...
- `agents_index_integrity_test.mjs`: release-safety coverage that parses the root `AGENTS.md` index and asserts every listed `AGENTS.md` path exists as a file in the repository
...
- keep each lane's explicit `tests` list in `test-lane-catalog.json` aligned with lane policy and test metadata; deterministic lanes should keep non-deterministic or heavy coverage out unless the lane explicitly allows it
