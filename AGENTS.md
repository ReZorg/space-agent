...
- `npm run ci:sanity`, `npm run test:unit`, `npm run test:integration`, `npm run test:e2e:desktop`, `npm run test:all`, `npm run test:release:preflight`, and `npm run test:nightly` provide the canonical release-quality lane commands backed by `tests/test-lane-catalog.json` and `tests/run-lane.mjs`, while `npm run test:stress:read` and `npm run test:stress:write` remain non-blocking stress or perf probes
- `.github/workflows/ci.yml` runs PR and main-branch release-safety lanes (sanity, unit, integration, desktop e2e) with failure artifact upload
- `.github/workflows/exhaustive-tests.yml` runs scheduled or manual exhaustive coverage lanes, including heavy desktop e2e and optional stress or perf informational runs with persisted artifacts for flake tracking
- `.github/workflows/release-desktop.yml` builds tagged desktop releases for Windows, macOS, and Linux on both x64 and arm64; automatic tag runs and manual `workflow_dispatch` reruns both require the selected `v*` tag to be on `main` history, and both skip only when a newer `v*` tag is already on `main` after it; release publish is gated by a release-preflight validation lane plus staged-asset verification (and optional manual packaged-asset smoke checks) before uploading clobbered artifacts selected by `packaging/release-asset-filters.yaml` with uniform `Space-Agent-<release version>-<platform>-<arch>.<extension>` asset names that collapse a redundant trailing `.0` patch to the project's normal two-segment release version
